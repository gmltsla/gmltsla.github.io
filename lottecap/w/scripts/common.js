...
function setMeta() {
	...
	// meta[1].content = "width=device-width, initial-scale=1.0";
	// $head.append(meta[1]);
	...
	// meta[1] = document.createElement("meta");
	// meta[1].name = "viewport";
	// meta[1].content = "width=device-width, initial-scale=1.0, minimum-scale=0, maximum-scale=1.0, user-scalable=no";
	// $head.append(meta[1]);
	...
	meta[2].name = "format-detection",
	meta[2].content = "telephone=no";
	$head.append(meta[2]);
}
	...
	$(".ui-carousel").each(function (idx, obj) {
		...
		for (var i = 0; i < classes.length; i++) {
			...
			if (classes[i].match(/^interval-/)) {
				...
				autoplay = JSON.parse("{\"delay\": " + interval + "}");
			}
